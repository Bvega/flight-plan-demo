 ...
 *
 * LAN helper:
 * - GET /api/lan      -> returns { port, ips: [...] } (IPv4 LAN candidates)
 */
...
function getLanIpv4s() {
  ...
  for (const name of Object.keys(nets)) {
    for (const net of nets[name] || []) {
      // Only IPv4, non-internal (so not 127.0.0.1)
...
const server = http.createServer((req, res) => {
  ...
  // LAN helper: give front-end a list of candidate LAN IPs + port
  if (url === "/api/lan" && req.method === "GET") {
    const payload = { port: Number(PORT), ips: getLanIpv4s() };
    res.writeHead(200, {
      "Content-Type": "application/json; charset=utf-8",
      "Cache-Control": "no-store",
    });
...
server.listen(PORT, "0.0.0.0", () => {
  ...
  console.log(`LAN helper: /api/lan`);
});
